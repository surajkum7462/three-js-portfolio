    ...
    <div className="relative min-h-screen bg-gradient-to-tr from-gray-900 via-purple-900 to-black text-white py-16 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid gap-12 px-6 md:grid-cols-2 lg:grid-cols-3 z-10 relative">
        {projects.map((project, index) => (
          <motion.div
            key={index}
            className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-6 border border-purple-500/20 hover:shadow-purple-700/50 transform transition-transform duration-500 hover:-translate-y-2 hover:scale-105 flex flex-col h-full"
            data-aos="fade-up"
            data-aos-delay={index * 100}
          >
            <img
              src={project.image}
              alt={project.title}
              className="w-full h-40 object-cover rounded-xl mb-4 border border-white/20 shadow-md"
            />
            <h3 className="text-2xl font-bold text-cyan-300 mb-2 drop-shadow-sm">
              {project.title}
            </h3>
